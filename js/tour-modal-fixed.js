    ...
    viewDetailsButtons.forEach((button) => {
        button.addEventListener('click', function(e) {
            ...
            if (tourId && tourData[tourId]) {
                ...
                if (modalPriceValue) modalPriceValue.textContent = tour.price;
                
                // Set the booking link with more detailed parameters
                if (bookTourBtn) {
                    const bookingUrl = `booking.html?booking=tour&tour=${tourId}&title=${encodeURIComponent(tour.title)}&price=${encodeURIComponent(tour.price)}`;
                    bookTourBtn.href = bookingUrl;
                    
                    // Add click event to booking button for better tracking
                    bookTourBtn.onclick = function(e) {
                        // Optional: Add analytics or tracking here
                        console.log(`Booking initiated for: ${tour.title}`);
